...
export async function up(knex: Knex): Promise<void> {
  await knex.schema.raw(`
    CREATE TRIGGER trg_user_books_audit
    ON user_books
    ...
    BEGIN
      ...
      BEGIN
        ...
        VALUES ('user_books', @operation_type, @record_id, GETDATE());
      END
    END;
  `);

  await knex.schema.raw(`DISABLE TRIGGER trg_user_books_audit ON user_books;`);
}

export async function down(knex: Knex): Promise<void> {
  await knex.schema.raw(`DROP TRIGGER IF EXISTS trg_user_books_audit;`);
}
