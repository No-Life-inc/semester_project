...
export async function up(knex: Knex): Promise<void> {
  await knex.schema.raw(`
    CREATE TRIGGER trg_user_book_tags_audit
    ON user_book_tags
    ...
    BEGIN
      ...
      BEGIN
        ...
        VALUES ('user_book_tags', @operation_type, @record_id, GETDATE());
      END
    END;
  `);

  await knex.schema.raw(`DISABLE TRIGGER trg_user_book_tags_audit ON user_book_tags;`);
}

export async function down(knex: Knex): Promise<void> {
  await knex.schema.raw(`DROP TRIGGER IF EXISTS trg_user_book_tags_audit;`);
}
