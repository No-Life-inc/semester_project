  ...
  await knex.schema.raw(`
    ...
    BEGIN
      ...
      BEGIN
        ...
        VALUES ('users', @operation_type, @record_id, GETDATE());
      END
    END;
  `);

  await knex.schema.raw(`DISABLE TRIGGER trg_users_audit ON users;`);
}

export async function down(knex: Knex): Promise<void> {
  await knex.schema.raw(`DROP TRIGGER IF EXISTS trg_users_audit;`);
}
