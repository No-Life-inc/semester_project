...
async function seedUsers() {
  ...
  try {
    ...
    await session.run(`MATCH (:Collection)-[r:CONTAINS_BOOK]->(:Book) DELETE r`);
    ...
    // Seed users
    const users = [
      { name: "User 1", email: "test_email@example.com", password: await bcrypt.hash(DEFAULT_PASSWORD, 10) },
      { name: "User 2", email: "test_password@example.com", password: await bcrypt.hash(DEFAULT_PASSWORD, 10) },
      { name: "Test Testsen", email: "test@test.com", password: await bcrypt.hash(DEFAULT_PASSWORD, 10) },
    ];
    ...
    for (const collection of collections) {
      const result = await session.run(
        `MATCH (u:User {email: $userEmail}) CREATE (c:Collection {name: $name})-[:BELONGS_TO]->(u) RETURN c`,
        { name: collection.name, userEmail: collection.userEmail }
      );
    ...
    if (books.length === 0) {
      console.log("No books found in the database. Cannot proceed with associations.");
      return;
    }
    ...
    const perUserBooks = Math.floor((books.length - sharedBooksCount) / users.length);
    ...
    const perCollectionBooks = Math.floor((books.length - sharedCollectionsBooksCount) / collections.length);
    ...
    for (let i = sharedCollectionsBooksCount; i < books.length; i++) {
        const collectionIndex = Math.floor((i - sharedCollectionsBooksCount) / perCollectionBooks);
        const collection = collectionNodes[collectionIndex % collectionNodes.length];
        await session.run(
        `MATCH (c:Collection {name: $collectionName}), (b:Book {isbn: $isbn}) CREATE (c)-[:CONTAINS_BOOK]->(b)`,
    ...
    //   const user = userNodes[index % userNodes.length]; // Round-robin user assignment
    //   await session.run(
    //     `MATCH (u:User {email: $userEmail}), (b:Book {isbn: $isbn}) CREATE (u)-[:HAS_BOOK]->(b)`,
    //     { userEmail: user.email, isbn: book.isbn }
    //   );
    // }
    // console.log("User-book associations created.");

    // // Associate collections with books
    // for (const [index, book] of books.entries()) {
    //   const collection = collectionNodes[index % collectionNodes.length]; // Round-robin assignment
    //   await session.run(
    //     `MATCH (c:Collection {name: $collectionName}), (b:Book {isbn: $isbn}) CREATE (c)-[:CONTAINS_BOOK]->(b)`,
    //     { collectionName: collection.name, isbn: book.isbn }
    //   );
    // }
    // console.log("Collection-book associations created.");
    ...
    if (tags.length === 0) {
      console.log("No tags found in the database. Cannot proceed with tagging.");
      return;
    }
