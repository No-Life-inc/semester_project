...
import { getUserCollectionsByEmail, createCollection, updateCollection, deleteCollection } from "../../services/neo4j/CollectionService";
 ...
 *   - `reg.body.oldName`: The old name of the collection.
 *   - `req.user.email`: The email of the authenticated user.
 * @param {Response} res - The response object used to send the result or error.
 */
export const updateCollectionController = async (req: AuthenticatedRequest, res: Response) => {
    ...
    try {
        const collection = await updateCollection(email, name, oldName);
        res.json(collection);
    } catch (error) {
 ...
 * @param {AuthenticatedRequest} req - The request object containing:
 *  - `req.params.name`: The name of the collection to be deleted.
 * - `req.user.email`: The email of the authenticated user.
 * @param {Response} res - The response object used to send the result or error.
...
export const deleteCollectionController = async (req: AuthenticatedRequest, res: Response) => {
    ...
    try {
        await deleteCollection(email, name);
        res.status(204).end();
    } catch (error) {
