import {
  ...
  describe('Subject Routes - Get All Subjects Negative Tests', () => {
    test.each([
      ...
      ["should return error for non-numeric limit", "/subject?page=1&limit=NaN", 400],
  ...
  describe('Subject Routes - Get Subject By ID Negative Tests', () => {
    test.each([
      ...
      ["should return error for non-numeric ID", "/subject/NaN", 400],
      ["should return error for non-existent ID", "/subject/9999", 400],
    ])(
      "%s",
      async (description, route, expectedStatus) => {
        const response = await request(app).get(route);
