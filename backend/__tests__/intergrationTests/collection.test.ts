// collection.test.ts
jest.setTimeout(30000); // Sætter timeout til 30 sekunder
...
describe("Collection Service Tests", () => {
    describe("Positive Tests", () => {
        ...
        it("should fetch collections for a valid user", async () => {
            ...
            expect(collections.length).toBeGreaterThanOrEqual(0); // Kan være tomt eller have data
        });

        it("should update a collection with a valid id", async () => {
            await updateCollection(1, "Updated Collection Name");
            const collections = await getUserCollections(1);
            expect(collections.find(col => col.name === "Updated Collection Name")).toBeDefined();
        });
        ...
        it("should delete a collection for a valid user and collection id", async () => {
            ...
            const collections = await getUserCollections(1);
            expect(collections.find(col => col.id === collection.id)).toBeUndefined();
        });
    });

    describe("Negative Tests", () => {
        it("should throw an error if the user does not exist when creating a collection", async () => {
            await expect(createCollection("Invalid User Collection", 999)).rejects.toThrow("User not found");
        });
        ...
        it("should throw an error if the collection does not exist when updating", async () => {
            await expect(updateCollection(999, "Nonexistent Collection")).rejects.toThrow("Collection not found");
        });
        ...
        it("should throw an error if the user does not own the collection when deleting", async () => {
            ...
            await expect(deleteCollection(collection.id, 2)).rejects.toThrow("Unauthorized to delete this collection");
        });

        it("should throw an error if the collection does not exist when deleting", async () => {
            await expect(deleteCollection(999, 1)).rejects.toThrow("Collection not found");
        });

        it("should throw an error when creating a collection without a name", async () => {
            await expect(createCollection("", 1)).rejects.toThrow("Validation error");
        });

        it("should throw an error if collection name is null during creation", async () => {
            await expect(createCollection(null, 1)).rejects.toThrow("Validation error");
        });
        ...
        it("should throw an error if updating a collection without a name", async () => {
            ...
            await expect(updateCollection(collection.id, "")).rejects.toThrow("Validation error");
        });
        ...
        it("should throw an error if user ID is missing when deleting a collection", async () => {
            ...
            await expect(deleteCollection(collection.id, null)).rejects.toThrow("Invalid user ID");
        });
        ...
        it("should throw an error for invalid user ID format in deleteCollection", async () => {
            ...
            await expect(deleteCollection(collection.id, "invalid_id" as any)).rejects.toThrow("Invalid user ID");
        });
