...
const updateCollectionNegativeCases: UpdateCollectionNegativeTestCase[] = [
  ...
  ["a" as any, "Another Valid Name", ValidationError, "Invalid collection ID", "test_email@example.com"],
  [null, "Another Valid Name", ValidationError, "Invalid collection ID", "test_email@example.com"],
  [undefined, "Another Valid Name", ValidationError, "Invalid collection ID", "test_email@example.com"],
  [0, "Another Valid Name", ValidationError, "Invalid collection ID", "test_email@example.com"],
  [-1, "Another Valid Name", ValidationError, "Invalid collection ID", "test_email@example.com"],
  [-2, "Another Valid Name", ValidationError, "Invalid collection ID", "test_email@example.com"],
  [7, "Another Valid Name", NotFoundError, "Collection not found", "test_email@example.com"],
  [8, "Another Valid Name", NotFoundError, "Collection not found", "test_email@example.com"],
...
const deleteCollectionNegativeCases: DeleteCollectionNegativeTestCase[] = [
  ...
  [-2, "test_email@example.com", ValidationError, "Invalid collection ID"],
...
// Add Book to Collection Positive Tests
...
describe("addBookToCollection function negative tests", () => {
  test.each([
    [null, 1, 1, ValidationError, "Invalid email address"],
    ["", 1, 1, ValidationError, "Invalid email address"],
    ["  ", 1, 1, ValidationError, "Invalid email address"],
    [undefined, 1, 1, ValidationError, "Invalid email address"],
    ["test_email@example.com", 0, 1, ValidationError, "Invalid collection ID"],
    ["test_email@example.com", -1, 1, ValidationError, "Invalid collection ID"],
    ["test_email@example.com", -2, 1, ValidationError, "Invalid collection ID"],
    ["test_email@example.com", NaN, 1, ValidationError, "Invalid collection ID"],
    ["test_email@example.com", null, 1, ValidationError, "Invalid collection ID"],
    ["test_email@example.com", undefined, 1, ValidationError, "Invalid collection ID"],
    ["test_email@example.com", "a", 1, ValidationError, "Invalid collection ID"],
    ["test_email@example.com", 1, 0, ValidationError, "Invalid book ID"],
    ["test_email@example.com", 1, -1, ValidationError, "Invalid book ID"],
    ["test_email@example.com", 1, -2, ValidationError, "Invalid book ID"],
    ["test_email@example.com", 1, null, ValidationError, "Invalid book ID"],
    ["test_email@example.com", 1, undefined, ValidationError, "Invalid book ID"],
    ["test_email@example.com", 1, "a", ValidationError, "Invalid book ID"],
    ["test_email@example.com", 1, NaN, ValidationError, "Invalid book ID"],
    ...
    ["test_email@example.com", 3, 3, UnauthorizedError, "You are not authorized to add a book to this collection"],
    ["test@test.com", 3, 3, ValidationError, "Book already exists in the collection"],
  ])(
    "should throw an error (email: %s, collectionId: %s, bookId: %s, errorClass: %s, errorMessage: %s)",
    async (
      email: string | null | undefined ,
      collectionId: number | null | undefined | string,
      bookId: number | null | undefined | string,
      errorClass: typeof NotFoundError | typeof ValidationError | typeof UnauthorizedError,
      errorMessage: string
    ) => {
      await expect(
        addBookToCollection(email as string, collectionId as number, bookId as number)
...
// Remove Book from Collection Positive Tests
...
// Remove Book from Collection Negative Tests
...
const removeBookFromCollectionNegativeCases: RemoveBookFromCollectionNegativeTestCase[] = [
  ...
  ["test_email@example.com", null, 1, ValidationError, "Invalid collection ID"],
  ["test_email@example.com", undefined, 1, ValidationError, "Invalid collection ID"],
  ["test_email@example.com", "a" as any, 1, ValidationError, "Invalid collection ID"],
  ["test_email@example.com", 1, 0, ValidationError, "Invalid book ID"],
  ["test_email@example.com", 1, -1, ValidationError, "Invalid book ID"],
  ["test_email@example.com", 1, -2, ValidationError, "Invalid book ID"],
  ["test_email@example.com", 1, 101, NotFoundError, "Collection not found"],
  ["test_email@example.com", 1, 102, NotFoundError, "Collection not found"],
  ["test_email@example.com", 101, 1, NotFoundError, "Collection not found"],
  ["test_email@example.com", 102, 1, NotFoundError, "Collection not found"],
  ["test_email@example.com", 1, NaN, ValidationError, "Invalid book ID"],
  ["test_email@example.com", 1, null, ValidationError, "Invalid book ID"],
  ["test_email@example.com", 1, undefined, ValidationError, "Invalid book ID"],
  ["test_email@example.com", 1, "a" as any, ValidationError, "Invalid book ID"],
  ["non_existent_user@example.com", 1, 1, NotFoundError, "User not found"],
  ["test_email@example.com", 999, 1, NotFoundError, "Collection not found"],
...
describe("removeBookFromCollection function negative tests", () => {
  test.each(removeBookFromCollectionNegativeCases)(
    ...
    async (
      ...
      await expect(
        removeBookFromCollection(email as string, collectionId as number, bookId as number)
      ).rejects.toThrow(errorClass);
      await expect(
        removeBookFromCollection(email as string, collectionId as number, bookId as number)
      ).rejects.toThrow(errorMessage);
